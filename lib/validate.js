...
function parseAdventure(body) {
  ...
  // Month granularity, collected as two selects. <input type="month"> is not
  // implemented in Firefox — it degrades to a plain text box with no picker —
  // so a pair of selects is the only control that works everywhere.
  // A YYYY-MM string is still accepted, for anything posting directly.
  const rawMonth = str(body.dateMonth).trim();
  const rawYear = str(body.dateYear).trim();

  if (rawMonth || rawYear) {
    if (!rawMonth || !rawYear) {
      errors.date = 'Pick both a month and a year.';
    } else {
      const parsed = parseMonth(`${rawYear}-${rawMonth.padStart(2, '0')}`);
      if (parsed === undefined || parsed === null) errors.date = 'Pick a month and a year.';
      else out.date = parsed;
    }
  } else {
