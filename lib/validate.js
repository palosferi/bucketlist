...
function parseAdventure(body) {
  ...
  const rawDate = str(body.date).trim();
  if (!rawDate) {
    out.date = null;
  } else {
    const d = new Date(rawDate);
    if (Number.isNaN(d.getTime())) errors.date = 'That is not a valid date.';
    else out.date = d;
  }
...
module.exports = { ValidationError, parseAdventure, parseList, parseSignup, RESERVED_HANDLES };
